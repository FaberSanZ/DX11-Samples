...
public sealed class GameWindow : IDisposable
{
    ...
    public void Initialize(Config? config = null)
    {
        ...
        Rect windowRect = new()
        {
            Right = (int)config.Width,
            Bottom = (int)config.Height
        };
    ...
    {
        if (MessageReceived?.Invoke(_handle, message, wParam, lParam) == true)
            return 0;
    ...
    private static nint WindowProc(nint handle, uint message, nuint wParam, nint lParam)
    {
        ...
        if (_currentWindow._handle == 0)
            _currentWindow._handle = handle;
    ...
    private void UpdateClientSize()
    {
        ...
        if (!GetClientRect(_handle, out Rect rect))
            return;
    ...
    private static uint HighWord(nint value)
    {
        return (uint)((value.ToInt64() >> 16) & 0xFFFF);
    }
